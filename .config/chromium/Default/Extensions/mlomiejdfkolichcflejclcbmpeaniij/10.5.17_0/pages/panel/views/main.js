...
import __vite_glob_0_18 from './clear-cookies.js';
import __vite_glob_0_20 from './menu.js';
import __vite_glob_0_26 from './tracker-details.js';
import __vite_glob_0_22 from './protection-status.js';
import __vite_glob_0_23 from './report-category.js';
import __vite_glob_0_25 from './report-form.js';
import __vite_glob_0_24 from './report-confirm.js';
import __vite_glob_0_27 from './whotracksme.js';
import __vite_glob_0_21 from './pause-assistant.js';
...
async function togglePause(host, event) {
  ...
  if (paused) {
    const pausedHostname = Object.keys(options.paused).sort((a, b) => b.localeCompare(a)).find((domain) => stats.hostname.endsWith(domain));
    store.set(options, {
...
const Main = {
  ...
  globalPause: ({ options }) => store.ready(options) && options.paused[GLOBAL_PAUSE_ID],
  ...
  render: ({
    ...
    <template layout="column grow relative">
      ${store.ready(options, managedConfig) && html`
        ${options.terms && html`
          <ui-header>
            ${store.ready(stats) && managedConfig.disableUserControl && html`<ui-text type="label-m">${stats.displayHostname}</ui-text>`}
            <ui-icon name="logo" slot="icon" layout="size:2.5"></ui-icon>
            ${!managedConfig.disableUserControl && html`
              <ui-action slot="actions">
                <a href="${router.url(__vite_glob_0_20)}" data-qa="button:menu">
                  <ui-icon name="menu" color="primary"></ui-icon>
                </a>
              </ui-action>
            `}
          </ui-header>
          ${store.ready(stats) && !managedConfig.disableUserControl && html`
            <panel-actions hostname="${stats.displayHostname}">
              ...
              <panel-actions-button>
                <a href="${router.url(__vite_glob_0_23)}">
              ...
              ${!isWebkit() && html`
                <panel-actions-button>
                  <a href="${router.url(__vite_glob_0_18)}">
    ...
    "/pages/settings/index.html#@settings-website-details?domain=" + stats.hostname
          ...
          `}
        `}
        ${options.terms ? store.ready(stats) && !managedConfig.disableUserControl && html`
              <panel-pause
                ...
                >
                  <ui-tooltip position="bottom" slot="actions">
                    ...
                    <ui-action-button layout="size:4.5">
                      <a href="${router.url(__vite_glob_0_27)}">
    ...
    ([name, trackers]) => html`
      ...
      (tracker) => html`
        ...
        trackerId: tracker.id
                                ...
                                ${!paused && !globalPause && options.terms && !managedConfig.disableUserControl && html`
